...
const MovieDetailsSm = () => {
  ...
  const item = useSelector((state)=>state.moviesSlice?.nowPlayingHoverMovieVideo?.playingHoverMovieData)
...
<div
        ...
        >
             ...
             <div>
              <h3>{item?.overview}</h3>
               ...
               <div className='pt-[2em]'>
                <h2 className='text-[#999]  text-[1.23em] font-bold mr-[0.5em]'>{t("Title details")}</h2>
                <div className='grid grid-cols-12 mt-[0.5em] gap-x-[5%]'>
                  <div className='col-span-6 '>
                      <div>
                      <h3 className='text-[#999] mt-[0.5em] text-[1em] font-medium'>{t("Director")}</h3>
                       <div>
                       { movieDetails?.genres?.map((genre, index) => <h4>{genre?.name}</h4>
                      )}
                       </div>
                      </div>
                      <div>
                      <h3 className='text-[#999] mt-[0.5em] text-[1em] font-medium'>{t("Cast")}</h3>
                       <div>
                       {movieDetails?.genres?.map((genre, index) => <h4>{genre?.name}</h4>
                      )}
                       </div>
          ...
          <div className='w-full px-[1.5em] '>
            <h1 className='text-[#999] text-[1.2em] font-bold'>{t("More Like This")}</h1>
...
similarMovieData?.map((movie)=> {
     ...
     return(
      ...
      <img src={POSTER_CDN + movie?.poster_path}
      alt="" className='h-36 rounded-md sm:h-40' />
